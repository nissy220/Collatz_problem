...
#This function is equal to the derivative of phi.
...
def estimate_max_stopping_time_from_density(r, p, t_range): 
...
plt.plot(np.arange(1, len(collatz_t_max) + 1), collatz_t_max, 'o', label='Collatz sequences') 
...
error=(pred_t_max[:len(collatz_t_max)]-collatz_t_max) / collatz_t_max
plt.plot(np.arange(3,len(collatz_t_max)+1), np.abs(error[2:]), 'o')             
plt.xlabel('$log_{10}n$')   
